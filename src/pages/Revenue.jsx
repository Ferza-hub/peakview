...
import { DollarSign, TrendingUp, TrendingDown, Target, Plus, X, Check, Trash2 } from 'lucide-react'
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
} from 'recharts'
import { revenueHistory, transactions as initialTransactions, revenueGoal } from '../data/mockData'
...
const EMPTY_TX = { description: '', platform: 'youtube', amount: '', status: 'paid', date: new Date().toISOString().split('T')[0] }
...
export default function Revenue() {
  ...
  const addTransaction = () => {
    ...
    const tx = { ...newTx, id: Date.now(), amount: parseFloat(newTx.amount) }
    setTransactions(prev => [tx, ...prev])
    setShowAdd(false)
    setNewTx(EMPTY_TX)
    setSavedTx(true)
    setTimeout(() => setSavedTx(false), 2000)
  }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {savedTx && <span className="flex items-center gap-1.5 text-sm text-emerald-600 font-medium"><Check size={14} /> Transaction logged</span>}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-slate-900">Log Income</h3>
              <button onClick={() => setShowAdd(false)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. YouTube AdSense — June payout"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewTx(p => ({ ...p, platform: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-400"
                  >
                    {Object.entries(PLATFORM_META).map(([k, m]) => <option key={k} value={k}>{m.label}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select
                    value={newTx.status}
                    onChange={e => setNewTx(p => ({ ...p, status: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-400"
                  >
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={addTransaction} className="btn-primary flex-1 justify-center">Log Transaction</button>
              </div>
            </div>
          </div>
        </div>
