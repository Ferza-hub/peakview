...
import { Plus, Search, Zap, Clock, Play, MoreHorizontal, Cpu, Video, Users, Pencil, Trash2, Check, X } from 'lucide-react'
...
import { initialTracks, mediaFiles as defaultMedia } from '../data/editorData'
...
export default function Dashboard() {
  ...
  const createProject = () => {
    ...
    saveProjectData(id, { tracks: initialTracks, mediaFiles: defaultMedia, format: '16:9' })
    ...
    <div className="min-h-screen bg-[#080808] text-zinc-100 flex flex-col">
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={createProject} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors">
        ...
        <div className="grid grid-cols-3 gap-4">
          {/* New card */}
          <div onClick={createProject} className="aspect-video rounded-2xl border-2 border-dashed border-[#2A2A2A] hover:border-violet-600/60 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all group">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    {isRen ? (
                      <div className="flex items-center gap-1 flex-1" onClick={e=>e.stopPropagation()}>
                        <input ref={renameRef} value={renameVal} onChange={e=>setRenameVal(e.target.value)}
                          ...
                          className="flex-1 bg-[#1A1A1A] border border-violet-500/60 rounded px-2 py-0.5 text-sm text-zinc-100 focus:outline-none" />
                        <button onClick={()=>commitRename(p.id)} className="text-emerald-400 p-0.5"><Check size={14}/></button>
                        <button onClick={()=>setRenamingId(null)} className="text-zinc-500 p-0.5"><X size={14}/></button>
                      </div>
                    ) : (
                    ...
                    {!isRen && (
                      <div className="relative" onClick={e=>e.stopPropagation()}>
                        <button className="text-zinc-600 hover:text-zinc-400 shrink-0 p-1 rounded hover:bg-white/5 transition-colors" onClick={()=>setMenu(menu===p.id?null:p.id)}>
                          <MoreHorizontal size={16}/>
                        </button>
                        {menu===p.id && (
                          <div className="absolute right-0 top-6 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl shadow-2xl z-50 overflow-hidden min-w-[120px]">
                            <button onClick={e=>startRename(p,e)} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-zinc-300 hover:bg-[#252525] transition-colors"><Pencil size={12}/> Rename</button>
                            <button onClick={e=>{e.stopPropagation();setMenu(null);setConfirmDel(p)}} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-red-400 hover:bg-red-950/40 transition-colors"><Trash2 size={12}/> Delete</button>
                          </div>
                        )}
                      </div>
                    )}
