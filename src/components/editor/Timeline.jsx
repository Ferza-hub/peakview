...
import { getEntry } from '../../utils/fileRegistry'
...
// ── Waveform canvas ──────────────────────────────────────────────────────────
function WaveformCanvas({ mediaId, color, width, height }) {
  ...
  useEffect(() => {
    ...
    if (!entry?.waveform?.length) {
      // Deterministic pseudo-random fallback using mediaId chars
      const seed = mediaId.split('').reduce((a, c) => a + c.charCodeAt(0), 0)
      const barCount = Math.max(4, Math.floor(canvas.width / 3))
      ctx.fillStyle = color + '50'
      for (let i = 0; i < barCount; i++) {
        const h = ((Math.sin(i * 0.7 + seed) * 0.5 + 0.5) * 0.75 + 0.1) * canvas.height
        ctx.fillRect(i * (canvas.width / barCount), (canvas.height - h) / 2, Math.max(canvas.width / barCount - 1, 1), h)
      }
      return
    }

    const data  = entry.waveform
    const barW  = canvas.width / data.length
    ctx.fillStyle = color + '75'
    for (let i = 0; i < data.length; i++) {
      const h = data[i] * canvas.height * 0.88
      ctx.fillRect(i * barW, (canvas.height - h) / 2, Math.max(barW - 0.5, 1), h)
    }
  }, [mediaId, color, width, height])
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[88px] shrink-0 border-r border-[#1F1F1F]">
          ...
          {local.map(track => (
            <div
              key={track.id}
              className="flex items-center gap-1 px-2 bg-[#111] border-b border-[#1A1A1A]"
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden relative" ref={scrollRef}>
          <div style={{ width: totalDuration * pps + 48, position: 'relative' }}>
            {/* Ruler */}
            <div
              className="h-6 border-b border-[#1F1F1F] bg-[#0D0D0D] sticky top-0 z-20 cursor-pointer select-none"
              style={{ width: totalDuration * pps + 48 }}
            ...
            >
              ...
              {Array.from({ length: totalDuration }, (_, i) => i)
            ...
            {local.map(track => (
              ...
              >
                ...
                {track.clips.map(clip => {
                  ...
                  const clipH = (track.height || TYPE_H[track.type] || 36) - 8
                        ...
                        <div className="absolute inset-0 flex items-center gap-px px-1.5 overflow-hidden opacity-35 pointer-events-none">
                          {Array.from({ length: Math.max(3, Math.floor(width / 4)) }).map((_, i) => (
                            <div key={i} className="w-px rounded-full shrink-0"
                              style={{ height: `${25 + ((i * 13 + 7) % 60)}%`, backgroundColor: clip.color }} />
                          ))}
                        </div>
                      )}
